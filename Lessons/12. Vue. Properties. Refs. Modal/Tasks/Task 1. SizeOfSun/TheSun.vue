<template>
    <div class="sun-container">
        <div class="sun-container__block sun-container__block--sun">
            <h3>Нажми на солнце</h3><div class="sun-container__img" @click="reduceSun">
                <img src="/Lessons/12.%20Vue.%20Properties.%20Refs.%20Modal/Tasks/Task 1. SizeOfSun/assets/sun.svg"
                     class="sun-container__the-sun"
                     ref="sun"
                >
            </div>
        </div><div class="sun-container__block">
            <h3>Год:</h3><span>      {{ year }}</span><h3>Диаметр солнца:</h3><span>      {{ diameter }} км</span>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        reduceSun() {
            ...
            this.year += 500000;
            this.sunWidth = Math.round(this.sunWidth * 0.95);
        },
    },
    updated() {
        this.$refs.sun.style.width = `${this.sunWidth}px`;
    },
    // Конец
};
</script>
